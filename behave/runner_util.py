...
def parse_features_data(data, language=None):
    """
    Parse UIfeature files and return list of Feature model objects.
    Handles:

      * UIfeature file names, ala "alice.UIfeature"
      * UIfeature file locations, ala: "alice.UIfeature:10"

    :param feature_files: List of UIfeature file names to parse.
    :param language:      Default language to use.
    :return: List of UIfeature objects.
    """
    scenario_collector = FeatureScenarioLocationCollector()
    features = []
    feature = parser.parse_data(data, language=language)
    if feature:
        # -- VALID FEATURE:
        # SKIP CORNER-CASE: Feature file without any UIfeature(s).
        scenario_collector.feature = feature
    # -- FINALLY:
    if scenario_collector.feature:
        current_feature = scenario_collector.build_feature()
