...
class MyRunMonkeyProcess(multiprocessing.Process):
    ...
    def generate_html_report(self):
        allure_result = os.popen('cd ../test;pwd;allure generate monkey/allure-results-{0} -o monkey/allure-report-{1} --clean'.format(self.udid,self.udid))
    ...
    def run_pytest_shell(self):
        ...
        result = os.popen('cd ../test;source activate uiautomator2;pwd;'
                          'py.test steps/test_android_monkey.py --alluredir=monkey/allure-results-{0} --platform={1} --automation={2} '
    ...
    def run_pytest_shell_ios(self):
        print(self.udid, 'start')
        result = os.popen('cd ../test;source activate uiautomator2;pwd;'
                          'py.test steps/test_ios_monkey.py --alluredir=monkey/allure-results-{0} --platform={1} --automation={2} '
                          '--automationName={3} --device={4} --port={5} --bp={6} --capture=no'
                          .format(self.udid, self.platform, self.automation, self.automationName, self.udid, self.port,
                                  self.bp))
...
if __name__ == '__main__' :
    ...
    port = 10000
    bp = 20000
    idx = [MyRunMonkeyProcess(platform='android' , automation= 'uiautomator2' , automationName='UiAutomator2', udid= i) for i in md.get_all_devices()]
    for i in idx:
        i.port = port
        i.bp = bp
        i.start()
    ...
    iox = [MyRunMonkeyProcess(platform='ios' , automation= 'appium' , automationName='XCUITest', udid= x+'_'+y+'_'+z) for x , y , z in iphone_devices]
    for m in iox:
        m.port = port
        m.bp = bp
        m.start()
